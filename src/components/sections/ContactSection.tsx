...
export function ContactSection() {
    ...
    return (
        <StickyWrapper height="150vh" className="bg-black z-40">
            {(progress: MotionValue<number>) => {
                const headerOpacity = useTransform(progress, [0, 0.2], [0, 1]);
                const headerY = useTransform(progress, [0, 0.2], [30, 0]);
                
                const formOpacity = useTransform(progress, [0.1, 0.4], [0, 1]);
                const formY = useTransform(progress, [0.1, 0.4], [30, 0]);
                
                const socialOpacity = useTransform(progress, [0.2, 0.6], [0, 1]);

                return (
                    <div id="contact" className="relative w-full h-full flex flex-col justify-center bg-transparent px-4 sm:px-8 text-foreground">
                        <div className="mx-auto max-w-4xl text-center w-full">
                            ...
                            <motion.div
                                ...
                                className="mx-auto max-w-xl rounded-2xl border border-white/10 bg-white/5 p-8 md:p-12 backdrop-blur-md"
                            >
                                {status === "success" ? (
                                    <motion.div
